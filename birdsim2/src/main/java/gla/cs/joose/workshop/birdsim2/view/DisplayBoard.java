...
/*
 * class DisplayPanel extends JPanel { private static final long
 * serialVersionUID = 1L;
 * 
 * /** Repaints this Board and everything on it.
 * 
 * @param g The Graphics context on which this board is painted.
 * 
 ...
 * @param g The Graphics context on which this board is painted.
 * 
 * public void paint(Graphics g) { // Paint the board thisBoard.paint(g); //
 * Paint the pieces synchronized (allPieces) { for (Iterator<Piece> iter =
 * allPieces.iterator(); iter.hasNext();) { Piece piece = iter.next();
 * piece.paint(g, piece.getRectangle()); } } } } // end inner class DisplayPanel
 */
